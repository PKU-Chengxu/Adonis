...
CMD_DIR = File.realpath('../../source', __FILE__)

UNSET_CMD = "unset LD_PRELOAD"
...
def run_cmd_func(name, args=[], data=nil)
  args = [args] unless args.respond_to?(:flatten)
  ENV['LD_PRELOAD'] = "../source/mqtt-sn-pub-decompile-hooking.so"
  cmd = [ENV] + [CMD_DIR + '/' + name] + args.flatten.map {|i| i.to_s}
