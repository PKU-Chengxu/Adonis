...
# used for getting format string of printf fprintf
class ImcompleteWasmSSAEmulatorEngine(EmulatorEngine):

    def __init__(self, cfg, func_index2func_name=None):
    ...
    def emulate_one_instruction(self, instr, state, depth, has_ret, call_depth, quick = False):
        instruction_map = {
        ...
        if instr.group == 'Memory':
            ...
            return instr_obj.emulate_not_step_in(state, has_ret, self.ana.func_prototypes, self.func_index2func_name, self.data_section, self.ana, quick)
