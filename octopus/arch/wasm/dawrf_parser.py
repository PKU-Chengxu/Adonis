...
def traverse_dwarf_info(dwarf_info):
    # Function get_source_location_by_addr will go over all the dwarf_info for each invoke.
    # This is very inefficient. So we traverse dwarf_info and store necessary info to avoid 
    # freguent parsing. This function will return all entries in the dwarf_info in an ascending order.
    ret = []
    for CU in dwarf_info.iter_CUs():
        # First, look at line programs to find the file/line for the address
        lineprog = dwarf_info.line_program_for_CU(CU)
        for entry in lineprog.get_entries():
            ...
            filename = lpe_filename(lineprog, entry.state.file)
            line = entry.state.line
            column = entry.state.column
            ret.append([address, filename, line, column].copy())
