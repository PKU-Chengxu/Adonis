...
class CPredefinedFunction:
    ...
    def emul(self, state, param_str, return_str, data_section, analyzer, quick=False):
        ...
        if self.name == 'printf' or self.name == 'iprintf' or self.name == '__small_printf':
            if quick:
                # in the quick mode, we do not care this instruction
                pass
            # has to use as_long()
            else:
                ...
                pattern = C_extract_string_by_mem_pointer(
                    pattern_p, data_section, state.symbolic_memory, quick)
                    ...
                    if isinstance(middle_p, BitVecRef) and not isinstance(middle_p, BitVecNumRef):
                        ...
                        if cur_pattern[-1] == 's':
                            tmp_data = str(C_extract_string_by_mem_pointer(
                                middle_p, data_section, state.symbolic_memory, quick))
            ...
            the_string = C_extract_string_by_mem_pointer(
                mem_pointer, data_section, state.symbolic_memory, quick)
            ...
            src_string = C_extract_string_by_mem_pointer(
                src, data_section, state.symbolic_memory, quick)
            ...
            dest_string = C_extract_string_by_mem_pointer(
                dest, data_section, state.symbolic_memory, quick)
            ...
            str1 = C_extract_string_by_mem_pointer(
                str1_p, data_section, state.symbolic_memory, quick)
            str2 = C_extract_string_by_mem_pointer(
                str2_p, data_section, state.symbolic_memory, quick)
            ...
            if self.name == 'strchr':
                ...
                needle = C_extract_string_by_mem_pointer(
                    needle_p, data_section, state.symbolic_memory, quick)
            haystack = C_extract_string_by_mem_pointer(
                haystack_p, data_section, state.symbolic_memory, quick)
        ...
        if not manually_constructed and return_str:
            ...
            tmp_bitvec = getConcreteBitVec(return_str,
                                           self.name + '_ret_' + return_str + '_' + cur_func + '_' + str(
                                               state.instr.offset))
