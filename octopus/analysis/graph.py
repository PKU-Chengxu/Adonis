...
def insert_edges_to_graph(graph, edges, call=False):
    from octopus.syspath.util import is_back_edge
    ...
    for edge in edges:
        ...
        if edge.type == EDGE_UNCONDITIONAL:
            graph.edge(edge.node_from, edge.node_to, color='blue', penwidth=penwidth)
        elif edge.type == EDGE_CONDITIONAL_TRUE:
            graph.edge(edge.node_from, edge.node_to, color='green', penwidth=penwidth)
        elif edge.type == EDGE_CONDITIONAL_FALSE:
            graph.edge(edge.node_from, edge.node_to, color='red', penwidth=penwidth)
        elif edge.type == EDGE_FALLTHROUGH:
            graph.edge(edge.node_from, edge.node_to, color='cyan', penwidth=penwidth)
        elif edge.type == EDGE_CALL and call:
            graph.edge(edge.node_from, edge.node_to, color='yellow', penwidth=penwidth)
        else:
            graph.edge(edge.node_from, edge.node_to, color='green', penwidth=penwidth)
...
class CFGGraph(Graph):
    ...
    def view_functions(self, only_func_name=None, view=True, simplify=False, call=False, ssa=False, color='grey'):
        ...
        if only_func_name:
            if hasattr(self, "mapper"):
                functions = [func for func in self.cfg.functions if self.get_func_readable_name(func) in only_func_name]
            else:
    ...
    def get_func_readable_name(self, func):
        if "$" in func.name:
            ...
            func_name = self.mapper[func_idx][1:]
        else:
            func_name = func.name  
        return func_name
